...
二次验证码本是日常安全的基本需求，但市面上不少 2FA 应用并不省心：

1. **有的要收费** — 订阅、会员或「高级功能」，而显示一个 TOTP 码本应是简单、免费的能力。
2. **有的太重** — 注册账号、登录、云同步、广告和一堆设置，添第一个令牌之前就要折腾很久。

TwoFactorAuth 走另一条路：**扫码或粘贴，然后看码**。数据留在本机，不注册、不付费、不依赖云端，只做一件小事并把它做好。
